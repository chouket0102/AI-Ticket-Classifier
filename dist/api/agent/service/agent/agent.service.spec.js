...
describe('AgentService', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        _reactagent.ReactAgent.mockImplementation(()=>mockReactAgent);
        mockRedisService = {
            subscribe: jest.fn().mockReturnValue((0, _rxjs.of)(JSON.stringify({
                data: {
                    id: 'test-id',
                    type: 'ai',
                    content: 'Test chunk'
        ...
        const module = await _testing.Test.createTestingModule({
            providers: [
                ...
                {
                    provide: _reactagent.ReactAgent,
        ...
        service = module.get(_agentservice.AgentService);
    });
    describe('analyzeTicket', ()=>{
        ...
        it('should throw BadRequestException on agent error', async ()=>{
            ...
            await expect(service.analyzeTicket({
                threadId: 'thread-3',
    ...
    describe('chat', ()=>{
        ...
        it('should throw BadRequestException on error', async ()=>{
            ...
            await expect(service.chat(messageDto)).rejects.toThrow(_common.BadRequestException);
        });
    });
    describe('stream', ()=>{
        it('should successfully stream messages', async ()=>{
            const mockChunk = new _messages.AIMessageChunk({
                content: 'Test chunk',
                id: 'test-id'
            });
            mockReactAgent.stream.mockImplementation(()=>{
                return Promise.resolve(createMockAsyncIterableStream([
                    mockChunk
                ]));
            });
            ...
            const emitted = await (0, _rxjs.firstValueFrom)(observable.pipe((0, _operators.toArray)()));
        ...
        it('should handle stream errors', async ()=>{
            mockRedisService.subscribe.mockReturnValueOnce((0, _rxjs.of)(JSON.stringify({
                type: 'error',
                data: {
            ...
            const observable = await service.stream(sseMessageDto);
            const emitted = await (0, _rxjs.firstValueFrom)(observable);
            expect(emitted).toEqual({
                type: 'error',
                data: {
                    message: 'Stream error'
    ...
    describe('getHistory', ()=>{
        ...
        it('should throw BadRequestException when fetching history fails', async ()=>{
            ...
            await expect(service.getHistory('test-thread-id')).rejects.toThrow(_common.BadRequestException);
            expect(mockReactAgent.getHistory).toHaveBeenCalledWith('test-thread-id');
        });
    });
});
function createMockAsyncIterableStream(chunks) {
    return {
