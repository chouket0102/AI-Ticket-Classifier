...
const _historicalservice = require("./historical.service");
describe('HistoricalService', ()=>{
    let service;
    beforeEach(()=>{
        service = new _historicalservice.HistoricalService();
    });
    ...
    it('should return matching tickets for database query', async ()=>{
        ...
        expect(results.some((t)=>t.description.toLowerCase().includes('database'))).toBe(true);
    });
    it('should return matching tickets for security query', async ()=>{
        ...
        expect(results.length).toBeGreaterThan(0);
        expect(results.some((t)=>t.category === 'Security')).toBe(true);
    });
    it('should return matching tickets for VPN query', async ()=>{
        const results = await service.query('VPN connectivity issues');
        expect(results.length).toBeGreaterThan(0);
        expect(results.some((t)=>t.description.toLowerCase().includes('vpn'))).toBe(true);
    });
...
//# sourceMappingURL=historical.service.spec.js.map